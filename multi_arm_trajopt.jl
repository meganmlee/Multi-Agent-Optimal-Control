...
# --- Helper Function for Pose Jacobian ---
# This function calculates the pose parameters [r; p] for a specific link
# based ONLY on the joint angles q_r of its robot. This is what we differentiate.
function get_link_pose_params(q_r::AbstractVector, link_idx::Int, robot_idx::Int, p_global::NamedTuple)
    # Extract parameters needed for FK
    base_pos = p_global.base_positions[robot_idx]
    link_lengths = p_global.link_lengths

    # Calculate all poses for this robot
    poses_r = forward_kinematics(q_r, base_pos, link_lengths)

    # Get pose for the specific link
    pos, rot = poses_r[link_idx]

    # Convert RotMatrix to MRP SVector
    mrp = Rotations.MRP(rot)
    mrp_vec = Rotations.params(mrp)

    # Return the concatenated pose parameters [r; p]
    return vcat(pos, mrp_vec) # Returns a 6-element vector
end
